...
Brief explanations of the EGG metadata categories can be found by running the
command below. For more detailed explainations of the EGG dataset, please refer
to the EGG manuscript in *Animal Microbiome* [https://doi.org/10.1186/s42523-025-00493-x](https://doi.org/10.1186/s42523-025-00493-x).
...
```
**NOTE: Before downloading table, it is recommended to increase the timeout limit for file downloads. Run the following code to increase to 10 minutes:**
``` r
options(timeout = 600)
```
...
To subset the EGG dataset, filter the metadata to your desired samples then 
run the command below. The subsetEGG() function will create a directory 
containing CSV files for the filtered metadata, feature table, and taxonomic
assignments.
